...
function setup_vendor() {
    ...
    export INVICTRIX_ROOT="$3"
    if [ ! -d "$INVICTRIX_ROOT" ]; then
        echo "\$INVICTRIX_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$INVICTRIX_ROOT/$OUTDIR" ]; then
        mkdir -p "$INVICTRIX_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$INVICTRIX_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$INVICTRIX_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$INVICTRIX_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$INVICTRIX_ROOT"/vendor/invictrix/build/tools/smali/baksmali.jar
        export SMALIJAR="$INVICTRIX_ROOT"/vendor/invictrix/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$INVICTRIX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$INVICTRIX_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$INVICTRIX_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$INVICTRIX_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$INVICTRIX_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$INVICTRIX_ROOT"/vendor/invictrix/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$INVICTRIX_ROOT"/"$OUTDIR"/radio
